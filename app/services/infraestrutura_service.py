...
# (chave, rótulo, cor, grupo) — fonte única das 17 métricas do notebook (cell-1):
# define ordem, cores e agrupamento da legenda (Saneamento × Espaços pedagógicos
# × Alimentação, esporte e convívio) do gráfico de métricas por escola.
...
METRIC_ESCOLA_FIELDS: list[tuple[str, str, str, str]] = [
    ...
    ("in_agua_potavel",              "Água potável",                 "#1F77B4", "Saneamento e serviços básicos"),
    ("in_energia_rede_publica",      "Energia (rede pública)",       "#FF7F0E", "Saneamento e serviços básicos"),
    ("in_esgoto_rede_publica",       "Esgoto (rede pública)",        "#8C564B", "Saneamento e serviços básicos"),
    ("in_lixo_servico_coleta",       "Coleta de lixo",               "#2CA02C", "Saneamento e serviços básicos"),
    ("in_banheiro",                  "Banheiro",                     "#17BECF", "Saneamento e serviços básicos"),
    ("in_banheiro_pne",              "Banheiro acessível (PNE)",     "#9467BD", "Saneamento e serviços básicos"),
    # --- Espaços pedagógicos ---
    ("in_biblioteca",                "Biblioteca",                   "#D62728", "Espaços pedagógicos"),
    ("in_sala_leitura",              "Sala de leitura",              "#E377C2", "Espaços pedagógicos"),
    ("in_laboratorio_ciencias",      "Laboratório de ciências",      "#BCBD22", "Espaços pedagógicos"),
    ("in_laboratorio_informatica",   "Laboratório de informática",   "#4C78A8", "Espaços pedagógicos"),
    ("in_sala_multiuso",             "Sala multiuso",                "#F58518", "Espaços pedagógicos"),
    ("in_sala_atendimento_especial", "Sala de atendimento especial", "#54A24B", "Espaços pedagógicos"),
    # --- Alimentação, esporte e convívio ---
    ("in_cozinha",                   "Cozinha",                      "#B279A2", "Alimentação, esporte e convívio"),
    ("in_refeitorio",                "Refeitório",                   "#72B7B2", "Alimentação, esporte e convívio"),
    ("in_quadra_esportes",           "Quadra de esportes",           "#EECA3B", "Alimentação, esporte e convívio"),
    ("in_patio_coberto",             "Pátio coberto",                "#FF9DA6", "Alimentação, esporte e convívio"),
    ("in_auditorio",                 "Auditório",                    "#9D755D", "Alimentação, esporte e convívio"),
]
COR_AUSENTE = "#E5E5E5"
...
PAINEL_DESCRICAO = "painel_infraestrutura"
MAPA_DESCRICAO = "mapa_infraestrutura"
ANALISE_TEMPORAL_DESCRICAO = "analise_temporal_infraestrutura"
...
class InfraestruturaService:
    """Serviço de infraestrutura: monta painéis, mapas e gráficos de evolução temporal."""

    def __init__(self, repository: InfraestruturaRepository):
    ...
    async def build_painel(
        ...
        """Monta o painel de infraestrutura: gráficos + cards.
        ...
        (
            records,
            total_escolas_com_infraestrutura_record,
        ...
        card_total_escolas_com_infraestrutura = self._build_total_escolas_card(
            total_escolas_com_infraestrutura_record,
        ...
        return {
            ...
            "data": {
                "graficos": {
                    ...
                    "card_total_escolas_com_infraestrutura": card_total_escolas_com_infraestrutura,
                    "tab_percent_infraestrutura": tab_percent,
                    "grafico_dependencia_infraestrutura": grafico_dependencia,
                    "grafico_tp_localizacao_infraestrutura": grafico_tp_localizacao,
                },
            },
        }
    ...
    async def build_painel_escolas(
        ...
        if variaveis:
            ...
            titulo_base = f"Métricas de infraestrutura por escola com {label}"
        else:
            titulo_base = "Métricas de infraestrutura por escola"
    ...
    async def build_mapa(
        ...
        """Lista as escolas com score (0–17) e classificação de infraestrutura
        calculados on-the-fly a partir de silver.infraestrutura_comum (não há
        tabela pré-computada gold). Sem transformação extra — apenas envelopa a
        saída do repository."""
    ...
    def _filtro_variaveis_label(variaveis: list[str], combine_or: bool) -> str:
        if combine_or:
            return "qualquer recurso de infraestrutura"
        if len(variaveis) == 1:
            return METRICS_BY_KEY.get(variaveis[0], variaveis[0])
        return "múltiplos recursos de infraestrutura selecionados"
    ...
    def _recorte_escola_label(
        ano: int | None,
        records: list[InfraestruturaEscola],
    ) -> str:
    ...
    def _build_tab_percent(
        self,
        records: list[InfraestruturaMunicipio],
        ano: int | None,
        variaveis: list[str],
    ...
    def _build_evolucao_temporal(
        self,
        records: list[InfraestruturaTemporal],
        metrica: str,
    ) -> dict:
    ...
    @staticmethod
    def _records_to_dataframe(records: list[InfraestruturaMunicipio]) -> pd.DataFrame:
        rows = [
            {
    ...
    @staticmethod
    def _temporal_to_dataframe(records: list[InfraestruturaTemporal]) -> pd.DataFrame:
        rows = [
            {
                "ano": r.ano,
    ...
    def _build_evolucao_temporal_figure(
        ...
        return ChartFactory.line_chart(
            ...
            titulo=titulo,
            y_axis_title="Percentual de infraestrutura",
        )
    ...
    def _build_dependencia_chart(
        self,
        records: list[InfraestruturaDependencia],
        variaveis: list[str],
        combine_or: bool,
        ano: int | None,
    ) -> dict:
    ...
    def _build_localizacao_chart(
        self,
        records: list[InfraestruturaLocalizacao],
        variaveis: list[str],
        combine_or: bool,
        ano: int | None,
    ) -> dict:
    ...
    def _build_metricas_por_escola_figure(
        ...
        ensino médio). Legenda manual agrupada por grupo de métrica e anotação
        n/17 ao final de cada barra. Os registros chegam ordenados por score
        DESC; o eixo Y é invertido para o maior score ficar no topo."""
        fig = go.Figure()
    ...
    def _temporal_dependencia_to_dataframe(
        records: list[InfraestruturaTemporalDependencia],
    ) -> pd.DataFrame:
        rows = [
            {
                "ano": r.ano,
